 ...
 *
 * **Every rung is read off the unit.** `parentParcelId`, `floorLevel`,
 * `unitNumber` and `prototypeUlpin` are all fields on the record the geometry
 * was built from. Nothing here parses the identifier to recover its parts, which
 * would be the tempting shortcut and would invert the actual dependency: the
 * segments were built *from* these values, not the other way round.
 */
...
function OwnershipHierarchy({ unit }: OwnershipHierarchyProps) {
  return (
    <section className="hierarchy" aria-label="Ownership hierarchy">
      ...
      <ol className="hierarchy-list">
        <Rung label="Parent parcel" value={unit.parentParcelId} mono />
        <Rung label="Floor" value={`Floor ${unit.floorLevel}`} />
        <Rung label="Unit" value={unit.unitNumber} />
      </ol>
